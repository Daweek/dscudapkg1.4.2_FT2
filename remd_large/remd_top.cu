...
void shuffleAtomIndex(Real3_t *pos_ar, Nmol_t Nmol);
int  placeUnitCubeFCC(Real3_t *pos_ar, Nmol_t Nmol);
void placeExpandToCellsize(Real3_t *posi_ar, Nmol_t Nmol, Real_t cellsize);
...
void mallocHost(Remd_t &remd, Simu_t &simu) {
   ...
   const MemSize_t vec3_size = sizeof(Real3_t) * remd.Nmol;
   const MemSize_t vec1_size = sizeof(Real_t)  * remd.Nmol;
   const MemSize_t ene_size  = sizeof(Real_t)  * Nrep * simu.step_exch;
   const MemSize_t temp_size = sizeof(Real_t)  * Nrep;
   const MemSize_t exch_size = sizeof(int)     * Nrep;

   const MemSize_t pvec3_size= sizeof(Real3_t *) * Ngpu;
   const MemSize_t pvec1_size= sizeof(Real_t  *) * Ngpu;
   const MemSize_t pint_size = sizeof(int     *) * Ngpu;
...
void mallocDev(Remd_t &remd, Simu_t &simu) {
   ...
   const MemSize_t extra = 0;
   const MemSize_t ar3_size = sizeof(Real3_t) * remd.Nmol * simu.Nrep_1dev + extra;
   const MemSize_t ar1_size = sizeof(Real_t)  * remd.Nmol * simu.Nrep_1dev + extra;
   const MemSize_t te_size  = sizeof(Real_t)  * simu.Nrep_1dev + extra;
   const MemSize_t en_size  = sizeof(Real_t)  * simu.step_exch * simu.Nrep_1dev + extra;
   const MemSize_t ch_size  = sizeof(int)     * simu.Nrep_1dev + extra;
#if defined(HOST_RUN)
   /*
    * Pseudo HOST_RUN
    */
...
// [input]
//------------------------------------------------------------------------------
int latticeSizeFCC(Nmol_t Nmol) {
...
//------------------------------------------------------------------------------
__global__ void
g_writePosition(Real3_t *d_pos_ar, int Nrep_1dev, int stored_offset, Nmol_t Nmol)  // <<<1,1>>>
{
  Real3_t i;

  for (int rep_i = 0; rep_i < Nrep_1dev; rep_i++) {
    printf("// %d th replica\n", rep_i);
    for (Nmol_t atom=0; atom<Nmol; atom++) {
...
initForce(Remd_t &remd) {
   ...
   for (Nmol_t i=0; i<remd.Nmol; i++) setZero( remd.h_foc_ar[i] );
...
//
//------------------------------------------------------------------------------
void shuffleAtomIndex(Real3_t *posi_ar, Nmol_t Nmol)
{
  int *index;
  int tmp;
  int shuffle;
  ...
  for (Nmol_t shuffle = 0; shuffle < Nmol*256; shuffle++) { /* shuffle */
    r = rand() % Nmol;
...
int placeUnitCubeFCC(Real3_t *posi, Nmol_t Nmol)
{
  debug_print(2, "Entering %s().\n", __func__);
  ...
  for (Nmol_t i=0; i<Nmol; i++) {
      posi[i].x = posi[i].y = posi[i].z = -1.0;
  }
  ...
  if (place_mark == NULL) {
    die("malloc(sizeof(int) * %ld) returned NULL, exit program.\n", Nmol);
  }
  ...
  for (int i=0; i<posi_tot; i++) {
    if (place_mark[i] == 1) fill_cnt++;
      ...
      die("wrong value of place_mark[%ld] in %s\n", i);
    }
  }
  
  if (fill_cnt != Nmol) {
    die("wrong value of fill_cnt = %ld in %s\n", fill_cnt);
  } else if (empty_cnt != remove_tot) {
    die("wrong value of empty_cnt = %ld in %s\n", empty_cnt);
  } else {
    printf("%s(): Nmol= %ld, posi_tot= %ld, fill_cnt= %ld, empty_cnt= %ld\n", __func__, Nmol, posi_tot, fill_cnt, empty_cnt);
    printf("%s(): latice_tot= %f , edge_grids= %ld\n", __func__, lattice_tot, edge_grids);
  }
  /* --> check errors */
  ...
  for (int i=0; i < edge_grids; i++) { ri = (Real_t)i;
	...
	for (int m=0; m < mol_per_lattice; m++) {
	  if (place_mark[posi_n] == 1) {
	    if (mol_n >= Nmol) {
	      die("confusing condition, mol_n(%ld) >= Nmol(%ld), i=%ld, j=%ld, k=%ld, m=%ld.\n",
		  mol_n, Nmol, i, j, k, m);
	    }
