...
copyTempMeas(int rep_i, CopyKind_t dir) {
    ...
    xcudaSetDevice( dev_i );
    switch (dir) {
    case H2D:
	xcudaMemcpy((void*)dev, (void*)host, size, cudaMemcpyHostToDevice);
	break;
    case D2H:
	xcudaMemcpy((void*)host, (void*)dev, size, cudaMemcpyDeviceToHost);
	break;
    default:
	die("Unknown parameter detected.");    
    }
...
copyEnergy(CopyKind_t dir, Remd_t &remd, Simu_t &simu) {
	...
	for (int gpu_i = 0; gpu_i < simu.Ngpu; gpu_i++) {
	    ...
	    xcudaMemcpy(d_energy, h_energy, copysize, cudaMemcpyHostToDevice);
	}
	break;
    case D2H:
	for (int gpu_i = 0; gpu_i < simu.Ngpu; gpu_i++) {
	    ...
	    d_energy = remd.d_energy[gpu_i];
	    xcudaSetDevice( gpu_i );
	    xcudaMemcpy( h_energy, d_energy, copysize, cudaMemcpyDeviceToHost);
	}
	break;
    default:
	die("Unknown CopyKind_t\n");
...
#     endif
	    xcudaSetDevice(gpu_i);
	    xcudaMemcpy(d_temp_ar, h_temp_ar, copysize, cudaMemcpyHostToDevice);
	}
	break;
    case D2H:
	for (int gpu_i = 0; gpu_i < simu.Ngpu; gpu_i++) {
	    ...
	    d_temp_ar = remd.d_temp_ar[gpu_i];
	    xcudaSetDevice( gpu_i);
	    xcudaMemcpy( h_temp_ar, d_temp_ar, copysize, cudaMemcpyDeviceToHost);
	}
	break;
    default:
	die("Unknown CopyKind_t\n");
...
copyExch(CopyKind_t dir, Remd_t &remd, Simu_t &simu) {
	...
	for (int gpu_i = 0; gpu_i < simu.Ngpu; gpu_i++) {
	    ...
	    d_exch_ar = remd.d_exch_ar[gpu_i];
	    xcudaSetDevice(gpu_i);
	    xcudaMemcpy(d_exch_ar, h_exch_ar, copysize, cudaMemcpyHostToDevice);
	}
	break;
    case D2H:
	for (int gpu_i = 0; gpu_i < simu.Ngpu; gpu_i++) {
	    ...
	    d_exch_ar = remd.d_exch_ar[gpu_i];
	    xcudaSetDevice(gpu_i);
	    xcudaMemcpy(h_exch_ar, d_exch_ar, copysize, cudaMemcpyDeviceToHost);
	}
	break;
    default:
	die("Unknown CopyKind_t\n");
